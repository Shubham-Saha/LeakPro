""" Last edit before Group testing """
...
class AttackRaMIA(AbstractMIA):
    ...
    def create_transformed_dataset(self: Self, num_samples: int) -> tuple:
        """Generate transformed ranges for selected audit samples, balanced between IN and OUT members.
            Perform group testing for all the transformed samples within the range.
        ...
        """
        # Uncomment these lines for baseline ramia
        ...
        self.selected_indices = np.concatenate([self.in_members, self.out_members]) # Uncomment this for no transformation or group testing and comment the above three lines
        ...
        for idx in tqdm(self.selected_indices, desc="Creating transformed ranges"):
            ...
            try:
                ...
                refined_samples = self.perform_group_testing(range_samples)

                # transformed_samples.append(range_samples) # Uncomment this for baseline ramia
                transformed_samples.append(refined_samples)
                
                true_labels.append(1 if idx in self.in_members else 0) # Append true labels (1 for IN, 0 for OUT)
    ...
    def perform_group_testing(self: Self, range_samples: list, n_clusters=2, n_representatives=5, m=2) -> list:
        ...
        """
        # If we have too few samples, return them as is
        if len(range_samples) <= n_representatives:
        ...
        # scaler = RobustScaler(quantile_range=(25, 75))  # Uses median and IQR instead of mean and std
        ...
        # plt.title('Diversity of Transformed Samples (PCA)')
        # plt.xlabel('PC1')
        # plt.ylabel('PC2')
        # plt.grid(True)
        # plt.show()
        ...
        kmeans = KMeans(n_clusters=n_clusters, init='random', n_init=20, random_state=42, max_iter=300)
        cluster_labels = kmeans.fit_predict(pca_features)
        centroids = kmeans.cluster_centers_

        # # Visualize clusters
        # plt.figure(figsize=(10, 8))
        # plt.scatter(pca_features[:, 0], pca_features[:, 1], c=cluster_labels, cmap='viridis', alpha=0.7, s=80)
        # plt.scatter(centroids[:, 0], centroids[:, 1], marker='X', s=200, c='red', edgecolors='black', linewidths=2)

        # # Add labels for centroids
        # for i, centroid in enumerate(centroids):
        #     plt.annotate(f'Centroid {i}', 
        #                 xy=(centroid[0], centroid[1]),
        #                 xytext=(centroid[0] + 0.1, centroid[1] + 0.1),
        #                 fontsize=12)

        # # Add counts per cluster
        # for i in range(n_clusters):
        #     count = np.sum(cluster_labels == i)
        #     plt.annotate(f'Cluster {i}: {count} pts',
        #                 xy=(0.05, 0.95 - i*0.05),
        #                 xycoords='axes fraction',
        #                 fontsize=10,
        #                 bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.8))

        # plt.title(f'K-means Clustering (K={n_clusters}) on PCA Features', fontsize=14)
        ...
        # # Calculate average distance between points to help set eps
        # # distances = pdist(pca_features)
        # # avg_dist = np.mean(distances)
        
        # nn = NearestNeighbors(n_neighbors=5).fit(pca_features)
        # distances, _ = nn.kneighbors(pca_features)
        # eps = np.median(distances[:, -1])
            
        #     # Set epsilon to a fraction of the average distance
        #     # eps = avg_dist * 0.5  # Adjust this multiplier as needed
            
        #     # Set min_samples based on dataset size
        #     # min_samples = max(2, len(range_samples) // 10)  # At least 2, or 10% of samples

        # min_samples = max(3, int(np.log2(len(range_samples))))
        ...
        # logger.info(f"DBSCAN found {num_clusters} clusters and {np.sum(cluster_labels == -1)} noise points")
            
        # # Visualize clusters
        # # plt.figure(figsize=(10, 8))
            
        # # # Create a colormap that reserves black for noise
        ...
        # #     plt.annotate(f'{cluster_name}: {count} pts',
        # #                 xy=(0.05, y_offset),
        # #                 xycoords='axes fraction',
        # #                 fontsize=10,
        # #                 bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.8))
        ...
        # # plt.title(f'DBSCAN Clustering (eps={eps:.2f}, min_samples={min_samples}) on PCA Features', fontsize=14)
        # # plt.xlabel('PC1', fontsize=12)
        # # plt.ylabel('PC2', fontsize=12)
        # # plt.grid(True, alpha=0.3)
        # # plt.legend()
        ...
        # # Step 3: Assignment Matrix & Cluster Means
        # valid_labels = [label for label in unique_labels if label != -1]

        # # Initialize assignment matrix
        # assignment_matrix = np.zeros((len(range_samples), len(valid_labels)), dtype=int)

        # # Fill the matrix
        # cluster_means = []
        # for idx, label in enumerate(valid_labels):
        #     cluster_mask = (cluster_labels == label)
        #     assignment_matrix[cluster_mask, idx] = 1
        #     cluster_means.append(np.mean(pca_features[cluster_mask], axis=0))
        ...
        for new_centroid in new_centroids:
            distances_to_new_centroid = np.linalg.norm(pca_features - new_centroid, axis=1)
            closest_idx = np.argmin(distances_to_new_centroid)
            representative_indices.append(closest_idx)
            
        # Ensure uniqueness and limit to n_representatives
        representative_indices = list(set(representative_indices))
        if len(representative_indices) > n_representatives:
            representative_indices = representative_indices[:n_representatives]
        
        refined_samples = [range_samples[idx] for idx in representative_indices]
        logger.info(f"Group testing found {len(refined_samples)} natural representatives")
        
        return refined_samples


        # #Step 5 & 6: Find samples closest to each centroid
        # representative_indices = []
        ...
        #     # Find distance from each sample to centroid
        #     distances = euclidean_distances([cluster_centroid], cluster_pca_features)[0]
                
        #     # Find index of closest sample to centroid
        #     original_indices = np.where(cluster_mask)[0]
        #     closest_idx = original_indices[np.argmin(distances)]
        #     representative_indices.append(closest_idx)
            
        # # If we couldn't find enough representatives, add more until we reach the desired number
        # if len(representative_indices) < n_representatives and len(range_samples) > len(representative_indices):
        #     # Get indices of samples not already selected
        #     remaining_indices = np.setdiff1d(np.arange(len(range_samples)), representative_indices)
        #     # Add samples randomly until we reach n_representatives or run out of samples
        #     additional_samples = min(n_representatives - len(representative_indices), len(remaining_indices))
        #     if additional_samples > 0:
        #         selected_additional = np.random.choice(remaining_indices, size=additional_samples, replace=False)
        #         representative_indices.extend(selected_additional)
            
        # # Step 7: Return the representative samples
        # refined_samples = [range_samples[idx] for idx in representative_indices]
            
        # # Log some information
        # logger.info(f"Group testing reduced {len(range_samples)} samples to {len(refined_samples)} representatives")

        # # Step 4: Select Representatives
        # representatives = []
        # for mean in cluster_means:
        #     # Find closest sample to cluster mean in PCA space
        #     distances = np.linalg.norm(pca_features - mean, axis=1)
        #     closest_idx = np.argmin(distances)
        #     representatives.append(range_samples[closest_idx])

        # # Step 5: Return Top N Representatives
        # return representatives[:n_representatives]
    ...
    def create_transformed_dataset_pkl(self, transformed_samples, selected_indices, true_labels, tmp_dir="./tmp"):
        ...
        This version handles the reduced number of samples per range after group testing.
        """
        os.makedirs(tmp_dir, exist_ok=True)
        ...
        for range_idx, (range_samples, class_label) in enumerate(zip(transformed_samples, original_classes)):
            start_idx = len(all_samples)
            all_samples.extend(range_samples)  # These are the group-tested representatives
            all_labels.extend([class_label] * len(range_samples))
            sample_to_range.extend([range_idx] * len(range_samples))
            range_to_samples[range_idx] = list(range(start_idx, start_idx + len(range_samples)))
    ...
    def prepare_attack(self:Self)->None:
        ...
        # num_samples = self.range_num_audit_samples # For baseline ramia
        num_samples = len(self.audit_data_indices) # Uncomment this for no transformation or group testing and comment the above line
        ...
        # Add logging to show the effect of group testing
        total_original_transforms = len(self.transformed_ranges) * self.num_transforms
        logger.info(f"Group testing reduced samples from {total_original_transforms} to {self.num_transformed_samples} total samples")
    ...
    def aggregate_range_scores(self):
        """Aggregate membership scores from representative samples for each range.
        
        Returns:
            np.ndarray: Array of aggregated scores for each range
        """
        range_scores = []
        for range_idx, sample_indices in self.metadata["range_to_samples"].items():
            ...
            # Calculate mean of all representative sample scores for this range
            range_sample_scores = [self.all_sample_scores[idx] for idx in sample_indices]
            range_scores.append(np.mean(range_sample_scores))
        
        return np.array(range_scores)
    ...
    def run_attack(self:Self) -> MIAResult:
        ...
        # Formatted masks to match the number of transformed samples for group testing
        if len(self.transformed_masks) != self.num_transformed_samples:
            raise ValueError(f"Mask length mismatch: {len(self.transformed_masks)} masks for {self.num_transformed_samples} samples")

        # Validate mask inheritance by checking a few samples for group testing
        for i in range(min(5, len(self.transformed_masks))):
            range_idx = self.metadata["sample_to_range"][i]
            original_idx = self.selected_indices[range_idx]
            assert np.array_equal(self.transformed_masks[i], self.in_indices_masks[original_idx]), \
                f"Mask inheritance failed for sample {i}, range {range_idx}"
        ...
        # Aggregate scores by range using simple averaging for group testing
        range_scores = self.aggregate_range_scores()

        # Comment below two lines for no transformation and group testing
        # self.estimate_distribution_separation() 
        # self.optimize_p()

        # Uncomment below lines for no transformation and baseline ramia
        # range_scores = []
        # for range_idx, sample_indices in self.metadata["range_to_samples"].items():
        #     range_sample_scores = self.all_sample_scores[sample_indices]
