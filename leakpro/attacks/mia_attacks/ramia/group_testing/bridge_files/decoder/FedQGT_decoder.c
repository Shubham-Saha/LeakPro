...
/*
 * BP decoder for Group Testing with
 * noiseless tests
 *
 * /
/* C Libraries */
...
//     for (i = 1; i < (1 << n); i++)
//     {
//         double product = 0; // check this for 0 or 1
//         for (j = 0; j < n; ++j)
//         {
//             if (i & (1 << j))
...
double CN_update(double *v1, double *v2, uint8_t degree, int test_value, bool is_0)
{
    ...
    //     return (log(0.5) + temp);
    // else
    //     return (maxstar(calculate_sum_product(v1, v2, degree - 1, 0), log(0.5) + temp));
    // Test positive: AT LEAST ONE node must be 1

    // IMPROVED TO HANDLE BINARY TEST OUTCOME
    else
    {
        if (is_0)
        {
            // Current node is 0, so at least one OTHER node must be 1
            // P(at least one other = 1) = 1 - P(all others = 0)
            double log_all_others_zero = 0.0;
            for (int i = 0; i < degree - 1; i++)
            {
                log_all_others_zero += v2[i]; // log P(other_i = 0)
            }
            
            // Use numerical stable computation
            if (log_all_others_zero < -20.0)
            {
                return 0.0; // All others very likely to be 0 // = log(1) = certainty that at least one other node is 1
            }
            else
            {
                double prob_all_others_zero = exp(log_all_others_zero);
                if (prob_all_others_zero >= 1.0)
                {
                    return -INFINITY; // Impossible case
                }
                return log(1.0 - prob_all_others_zero);
            }
        }
        else
        {
            // Current node is 1, constraint automatically satisfied
            return 0.0;
        }
    }
}
...
void decode(uint8_t *test_outcome, uint8_t *DEC, double *SOFT_SCORES, double *prevalence, uint16_t maxIter)
{
    ...
    for (size_t j = 0; j < max_dv; j++)
    {
        for (size_t i = 0; i < n; i++)
        {
            ...
            // IMPROVED: Prevalence for initialization
            double temp_prev = prevalence[i];
            L_vc[j][i][0] = log(1.0 - temp_prev);
            L_vc[j][i][1] = log(temp_prev);
        }
    }
    for (size_t j = 0; j < max_dc; j++)
    {
        for (size_t i = 0; i < t; i++)
        {
            for (size_t bit = 0; bit < 2; bit++)
                // L_cv[j][i][bit] = 0;
                L_cv[j][i][bit] = log(0.5); // IMPROVED: before any tests, each sample is 50% likely to be 0 and 50% likely to be 1
        }
    }
    ...
    while (maxIter-- > 0)
    {
        ...
        for (size_t i = 0; i < n; i++)
        {
            ...
            for (size_t l = 0; l < vn_deg[i]; l++)
            {
                ...
                for (size_t bit = 0; bit < 2; bit++)
                {
                    ...
                    {
                        if (r != l)
                            L_vc[l][i][bit] += L_cvi[bit][r]; // VNssh
                    }
                }
        ...
        for (size_t i = 0; i < n; i++)
        {
            ...
            // ADDED: Compute soft scores
            double temp_sum = maxstar(temp_0, temp_1);
            if (temp_1 == -INFINITY) {
                SOFT_SCORES[i] = 0.0;
            }
            else if (temp_0 == -INFINITY) {
                SOFT_SCORES[i] = 1.0;
            }
            else if (temp_sum > -INFINITY) {
                SOFT_SCORES[i] = exp(temp_1 - temp_sum);
            }
        ...
        for (size_t i = 0; i < t; i++)
        {
            ...
            for (size_t l = 0; l < cn_deg[i]; l++)
            {
                ...
                {
                    parity = 1;
                    break; // OR operation - one positive is enough
                }
            }
...
void BP_decoder(const uint16_t n_input, const uint16_t t_input, const uint8_t dvmax, const uint8_t dcmax,
                ...
                const double *prevalence_input, const uint16_t max_Iter_input, const uint8_t *cn_deg_input, const uint8_t *vn_deg_input, uint8_t *DEC, double *SOFT_SCORES)
{
    n = n_input;
    t = t_input;
    max_dv = dvmax;
    ...
    for (size_t i = 0; i < t; i++)
    {
        if ((test_outcome_input[i] == 0) || test_outcome_input[i] == 1)
    ...
    decode(test_outcome, DEC, SOFT_SCORES, prevalence, maxIter);
    free2d_si16(vn);
    free2d_si16(cn);
    free(test_outcome);
    free(prevalence);
